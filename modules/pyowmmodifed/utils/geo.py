...
class Point(Geometry):
    ...
    def bounding_square_polygon(self, inscribed_circle_radius_km=10.0):
        """
         ...
         :return: a `pyowmmodifed.utils.geo.Polygon` instance
         """
    ...
    def from_dict(cls, the_dict):
        ...
        :return: `pyowmmodifed.utils.geo.Point` instance
        """
...
class MultiPoint(Geometry):
    ...
    def from_points(cls, iterable_of_points):
        """
        Creates a MultiPoint from an iterable collection of `pyowmmodifed.utils.geo.Point` instances
        :param iterable_of_points: iterable whose items are `pyowmmodifed.utils.geo.Point` instances
    ...
    def from_dict(cls, the_dict):
        ...
        :param the_dict: the geoJSON dict
        :return: `pyowmmodifed.utils.geo.MultiPoint` instance
        """
        geom = geojson.loads(json.dumps(the_dict))
...
class Polygon(Geometry):
    ...
    def from_dict(cls, the_dict):
        ...
        :param the_dict: the geoJSON dict
        :return: `pyowmmodifed.utils.geo.Polygon` instance
        """
        geom = geojson.loads(json.dumps(the_dict))
    ...
    def from_points(cls, list_of_lists):
        ...
        `pyowmmodifed.utils.geo.Point` instances
...
class MultiPolygon(Geometry):
    ...
    `pyowmmodifed.utils.geo.Point` documentation for details
    ...
    def from_dict(cls, the_dict):
        ...
        :param the_dict: the geoJSON dict
        :return: `pyowmmodifed.utils.geo.MultiPolygon` instance
        """
        geom = geojson.loads(json.dumps(the_dict))
        result = MultiPolygon([
            [[[0, 0], [0, 0]]],
    ...
    def from_polygons(cls, iterable_of_polygons):
        ...
        :param iterable_of_polygons: list of `pyowmmodifed.utils.geo.Point` instances
...
class GeometryBuilder:
    ...
    def build(cls, the_dict):
        """
        Builds a `pyowmmodifed.utils.geo.Geometry` subtype based on the geoJSON geometry type specified on the input dictionary
        :param the_dict: a geoJSON compliant dict
        :return: a `pyowmmodifed.utils.geo.Geometry` subtype instance
