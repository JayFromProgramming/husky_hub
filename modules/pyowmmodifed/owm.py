...
class OWM:
    ...
    def agro_manager(self):
        """
        Gives a `pyowmmodifed.agro10.agro_manager.AgroManager` instance that can be used to read/write data from the
        Agricultural API.
        :return: a `pyowmmodifed.agro10.agro_manager.AgroManager` instance
        """
        return agro_manager.AgroManager(self.api_key, self.config)

    def airpollution_manager(self):
        """
        Gives a `pyowmmodifed.airpollutionapi30.airpollution_manager.AirPollutionManager` instance that can be used to fetch air
        pollution data.
        :return: a `pyowmmodifed.airpollutionapi30.airpollution_manager.AirPollutionManager` instance
        """
    ...
    def tile_manager(self, layer_name):
        """
        Gives a `pyowmmodifed.tiles.tile_manager.TileManager` instance that can be used to fetch tile images.
        :param layer_name: the layer name for the tiles (values can be looked up on `pyowmmodifed.tiles.enums.MapLayerEnum`)
        :return: a `pyowmmodifed.tiles.tile_manager.TileManager` instance
        """
        return tile_manager.TileManager(self.api_key, layer_name, self.config)

    def uvindex_manager(self):
        """
        Gives a `pyowmmodifed.uvindexapi30.uvindex_manager.UVIndexManager` instance that can be used to fetch UV data.
        :return: a `pyowmmodifed.uvindexapi30.uvindex_manager.UVIndexManager` instance
        """
        return uvindex_manager.UVIndexManager(self.api_key, self.config)

    def weather_manager(self):
        """
        Gives a `pyowmmodifed.weatherapi25.weather_manager.WeatherManager` instance that can be used to fetch air
        pollution data.
        :return: a `pyowmmodifed.weatherapi25.weather_manager.WeatherManager` instance
        """
        return weather_manager.WeatherManager(self.api_key, self.config)

    def geocoding_manager(self):
        """
        Gives a `pyowmmodifed.geocoding10.geocoding_manager.GeocodingManager` instance that can be used to perform direct
        and reverse geocoding
        :return: a `pyowmmodifed.geocoding10.geocoding_manager.GeocodingManager` instance
        """
